...
- **`data/products.json`** — every product: name/description/specs in
  ...
  photos, badges (`new`, `sale`, `bestseller`), and whether it's
  `featured` / `newArrival`.
...
Open `data/products.json`, find the product by its `name`, and edit:
...
- `price` is the original price. `discountPrice` is the sale price shown
  next to it with a strikethrough — set it to `null` if there's no discount:
  `"discountPrice": null`.

### Changing a description

Same file, same product — edit the three languages under `"description"`:

```json
"description": {
  "en": "Your new English description here.",
  "fr": "Votre nouvelle description en français.",
  "ar": "وصفك الجديد بالعربية هنا."
}
```

You can leave any language as-is if you only want to change one. The
`"specs"` field (materials, dimensions, closure type) works the same way,
as a list of short bullet strings per language.

### Adding real product photos

By default every product shows an elegant gold line-art placeholder — no
photo needed to launch. To use real photos instead:

1. Add your photo files inside `/assets/products/` (JPG or WEBP,
   ideally 1200×1500px, under 300KB each for fast loading).
2. In `data/products.json`, find the product and fill its `"images"` array:

```json
"images": [
  "assets/products/lune-doree-1.jpg",
  "assets/products/lune-doree-2.jpg",
  "assets/products/lune-doree-3.jpg"
]
```

That's it — the product card, the product page gallery (with clickable
thumbnails), and the shopping cart will all automatically show your
photos instead of the illustration. Leave `"images": []` empty for any
product you don't have photos for yet — it will keep showing the
placeholder automatically, and if a photo path is ever wrong or a file
goes missing, the site quietly falls back to the illustration instead of
showing a broken image icon.

To add a brand-new product, copy an existing entry inside the `products`
array in `data/products.json`, give it a unique `id` and `slug`, and edit
the fields above. No JavaScript or HTML editing is required — the site
reads this file on every page load.
